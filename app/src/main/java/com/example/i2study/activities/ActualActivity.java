...
public class ActualActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Получаем данные об уроках и выводим их
        generateActualLessons();

        name = findViewById(R.id.textViewName);
        String txtName = sessionManager.getUsername();
//        Можно достать роль
//        String txtName = sessionManager.getRole();
    ...
    public void generateActualLessons(){
        ...
        for (LessonModel lessonModel : lesson_list) {
            if (lessonModel.getDay().equals(getDayStringOld(new Date(System.currentTimeMillis()), Locale.ENGLISH))){
        ...
        if(lessonList.size() > 0){
            ...
            SimpleAdapter adapter = new SimpleAdapter(ActualActivity.super.getBaseContext(), data, R.layout.schedule_item, from, to);
            ...
            String[] from = {ATTRIBUTE_NAME_NUMBER};
            int[] to = {R.id.numberHolder};

            SimpleAdapter adapter = new SimpleAdapter(ActualActivity.super.getBaseContext(), data, R.layout.no_lessons, from, to);
            //Выводим список уроков
            listView = findViewById(R.id.listView);
            listView.setAdapter(adapter);
        }
